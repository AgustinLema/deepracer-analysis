...
def get_reward(params):
    ...
    MAX_SPEED = 3
    MAX_SPEED_CURVES_TURNING = 2.2
    if speed > MAX_SPEED *.66:
        speed_multiplier = 0.66 + (speed/MAX_SPEED) / 2
    elif speed >= MAX_SPEED *.33:
        speed_multiplier = 0.33 + (speed/MAX_SPEED)
    else:
        speed_multiplier = (speed/MAX_SPEED)/2
    ...
    if curve_state == "STRAIGHT":
        if curve_distance > CURVE_DISTANCE_THRESHOLD:
            ...
            reward *= 1.1 # TODO: Remove, it is to compensate that no reward is given at this point
    else:
